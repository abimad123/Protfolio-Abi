 ...
 document.addEventListener('DOMContentLoaded', function() {
     ...
     const flipCards = document.querySelectorAll('.certification-flip');

     function handleDesktopHover(card) {
         ...
         card.addEventListener('mousemove', function(e) {
             ...
             // If cursor is in top 30% of the card
     ...
     function handleFlipInteraction() {
         if (window.innerWidth > 991) {
             // Desktop logic
             flipCards.forEach(card => {
                 handleDesktopHover(card);
             });
         } else {
             // Mobile logic
             flipCards.forEach(card => {
             ...
             const updatedFlipCards = document.querySelectorAll('.certification-flip');
             updatedFlipCards.forEach(card => {
                 card.addEventListener('click', function() {
                     document.querySelectorAll('.certification-flip.active').forEach(activeCard => {
                         if (activeCard !== this) {
     ...
     function typeLogo() {
         logo.innerHTML = logoText.substring(0, logoIndex) + (logoIndex < logoText.length ? '<span class="cursor-blink">|</span>' : '');
         if (logoIndex < logoText.length) {
             logoIndex++;
             setTimeout(typeLogo, 120);
         } else {
             logo.innerHTML = logoText.substring(0, logoText.indexOf(" ")) + "<span> C G</span>";
         }
     }
     if (logo) typeLogo();

     // --- Typing Effect for Hero Section ---
     const heroText = "Abijith C G";
     const typingHero = document.getElementById("typing-hero");
     let heroIndex = 0;
     let typing = true;

     function typeHero() {
         if (!typingHero) return;
         if (typing) {
             typingHero.innerHTML = heroText.substring(0, heroIndex) + (heroIndex < heroText.length ? '<span class="cursor-blink">|</span>' : '');
             ...
             typingHero.innerHTML = heroText.substring(0, heroIndex) + '<span class="cursor-blink">|</span>';
             if (heroIndex > 0) {
                 heroIndex--;
